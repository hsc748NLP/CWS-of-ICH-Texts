...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        return ['B-tag', 'I-tag', 'E-tag', 'S-tag', "[CLS]", "[SEP]"]
...
def convert_examples_to_features_eval(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if len(tokens) >= max_seq_length - 1:
            tokens = tokens[0:(max_seq_length - 2)]
            labels = labels[0:(max_seq_length - 2)]
            textlist = textlist[0:(max_seq_length - 2)]

        ntextlist = ["[CLS]"] + textlist + ["[SEP]"]
   ...
   for i in sentence:
        if len(i)<509:
           result.append(i)
            ...
            result.extend(clip_list(i,509))  #防止用户的输入序列过长
   return result
...
def process_txt(raw_path,output,max_seq_length=128,eval_batch_size=8):
    task_name=r'ner'
    bert_model=r'roberta_6/out'
    output_dir=r'roberta_6/out'
    # bert_model = r'out'
    # output_dir = r'out'
    ...
    eval_examples = processor.get_dev_examples(raw_path)  # 读取token格式文本
    ...
    generate_result(alltokens, y_true, y_pred, output)

def get_output(content):
    output_text=content.replace('B','').replace('I','').replace('E','/').replace('S','/')
    return output_text

def fenci(raw_path, outputpath, file):
    test=[]
    list3=[]
    tokens=[]
    f1=open('out/labeled_results.txt','r',encoding='utf-8')
    f2=open(raw_path,'r',encoding='utf-8')
    list1=f1.readlines()
    for i in list1:
        i=re.sub('\t','',i)
        list3.append(i)
    ...
    with open(outputpath+'/'+file+'_分词后.txt','w',encoding='utf-8')as fp:
        for i in combines:
            lines=get_output(i)
            fp.write(lines+'\n')
    # print(combines)
...
#     for i in filearray:
#        process_txt(i,output='temp_data',max_seq_length=max_seq_length,eval_batch_size=eval_batch_size)
#        fenci(resultpath,i.split('/')[1].replace('.txt',''))


def TCfenci_all(raw_path,resultpath,max_seq_length,eval_batch_size):
    process_txt(raw_path,output='./out',max_seq_length=max_seq_length,eval_batch_size=eval_batch_size)
    fenci(raw_path, resultpath, raw_path.split('\\')[-1].replace('.txt',''))
...
if __name__ == "__main__":
    ...
    # get_seq('data')
    # TCfenci_all(raw_path='data',resultpath='final_result',max_seq_length=512,eval_batch_size=32)
    print(os.getcwd())
    # process_txt(raw_path=r'../temp/蒙古族长调民歌_token.txt', output='../out/labeled_results.txt', max_seq_length=128, eval_batch_size=8)
    # TCfenci_all(raw_path=r'C:\Users\SkyHu\Desktop\data', resultpath=r'C:\Users\SkyHu\Desktop\out', max_seq_length=512, eval_batch_size=32)
